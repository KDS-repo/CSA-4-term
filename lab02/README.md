...
АЛУ выполняет арифметическо-логические операции над двумя операндами. Значение первого операнда берется из `_src1Val` в случае, если инструкция определяет валидный `_src1`. Значение второго операнда определяется либо значением `_imm`, если оно определено инструкцией, либо `_src2Val`. Возможно, что один или оба операндов не определены в инструкции, это значит, что вычисления АЛУ или не происходят, или игнорируются. Результат вычисления всегда записывается в поле `_addr` для инструкций типа `Itype::Ld` или `Itype::St`. Обозначим первый операнд А, а второй Б. Операции реализуемые АЛУ:
```
AluFunc::Add — А + Б.
AluFunc::Sub — А - Б.
